...
<style>
  ...
  <div class="form-box">
    <div class="form">
    <form class="search-form" action="searchtest.php" method="post">
      <label for="did"><span class="content-name">Patient ID</span></label>
      <input type="text" name="did" placeholder="Enter Patient ID" value="<?php echo isset($_GET['id']) ? htmlspecialchars($_GET['id']) : ''; ?>"><br>
...
<?php
  ...
  if (!empty($_POST["ref"])) {
      ...
      if (!mysqli_stmt_prepare($stmt,$sql)) {
        header("Location:searchtest.php?error=sqlerror1");
      }
      else {
        ...
        echo "</tbody></table>
          ...
          if (empty($_POST["did"])) {
            header("Location:searchtest.php?error=emptyfields");
          }
          elseif (empty($_POST["date"]) || empty($_POST["time"])) {
            ...
            $stmt= mysqli_stmt_init($conn);
            if (!mysqli_stmt_prepare($stmt,$sql)) {
              header("Location:searchtest.php?error=sqlerror");
            }
            else {
          ...
          if (!mysqli_stmt_prepare($stmt,$sql)) {
            ...
            echo "</tbody></table>
              ...
              elseif (empty($_POST["date"]) || empty($_POST["time"])) {
                ...
                $stmt= mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt,$sql)) {
                  header("Location:searchtest.php?error=sqlerror2");
                }
                else {
                  mysqli_stmt_bind_param($stmt, "sss", $did, $date, $time);
                  mysqli_stmt_execute($stmt);
